...
# ---------------------------------------------------------
# KGT v4.3 – Optimized Königsmann-Theory (ψ + Φ + Ω)
# ---------------------------------------------------------
...
# ------------------------------
# Optimierte Parameter v4.3
# ------------------------------
gamma_psi = 1.33                # milder ψ-Boost zur Kontrolle des Zentrums

eta0 = -0.25                    # stärkerer Abfall innen (optimiert)
eta1 = -0.09                    # außen bleibt wie v4.2
eta_trans = 12.0                # Übergangsradius

phi_amp = 3.9                   # minimale Absenkung reduziert Innenüberschuss
phi_switch = 7.0
phi_width = 3.8
osc_amp = 0.30
phi_log = 1.618034              # Goldener Schnitt

Omega_amp = 46000.0             # asymptotisches Plateau weiterhin stabil
rc = 6.5                        # stärkere core-suppression (optimiert)
...
# Ausgabe
# ---------------------------------------------------------
print("\n--- KGT v4.3 – Optimierter Andromeda-Fit ---\n")
...
plt.plot(r, v_sim, label="KGT v4.3 (optimiert)", linewidth=3, color="navy")
...
plt.title("Andromeda Rotationskurve – Königsmann-Theory v4.3")
plt.grid(alpha=0.4)
plt.legend()
plt.tight_layout()
plt.show()
